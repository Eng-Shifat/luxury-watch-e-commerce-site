/* ============================================================
   ...
   Self-contained. Does NOT depend on main.js or checkout.js.
   ============================================================ */
...
function showStep(n) {
  ...
  document.getElementById('co-back-btn').style.display  = n > 1 ? '' : 'none';
  document.getElementById('co-next-btn').style.display  = n < 3 ? '' : 'none';
  document.getElementById('co-order-btn').style.display = n === 3 ? '' : 'none';
...
function validateShipping() {
  ...
  if (phone && !/^(?:\+88|88)?01[3-9]\d{8}$/.test(phone)) {
    var el  = document.getElementById('co-phone');
    var err = el.nextElementSibling;
    el.classList.add('is-error');
...
function renderSidebar() {
  ...
  document.getElementById('co-sidebar-items').textContent = items + ' item' + (items > 1 ? 's' : '');
  document.getElementById('co-sidebar-total').textContent = '$' + total.toLocaleString();
}
...
function renderCustomerRecap() {
  ...
  document.getElementById('co-customer-recap').innerHTML =
...
async function placeOrder() {
  ...
  var mobNum    = document.getElementById('co-mobile-number') ? document.getElementById('co-mobile-number').value.trim() : '';
  var total     = cart.reduce(function (s, i) { return s + i.price * i.qty; }, 0);
  var items     = cart.reduce(function (s, i) { return s + i.qty; }, 0);
  var orderId   = 'ORD-' + Date.now().toString().slice(-6);
  var orderDate = new Date().toLocaleString('en-BD', { timeZone: 'Asia/Dhaka' });

  var paymentLabels = { cod: 'Cash on Delivery', bkash: 'bKash', nagad: 'Nagad', card: 'Card' };
  ...
  document.getElementById('co-flow').style.display    = 'none';
  document.getElementById('co-sidebar').style.display = 'none';
  document.getElementById('co-success-oid').textContent = orderId;
  document.getElementById('co-success-state').style.display = 'flex';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!cart.length) {
    document.getElementById('co-empty-state').style.display = 'block';
    document.getElementById('co-flow').style.display        = 'none';
    document.getElementById('co-sidebar').style.display     = 'none';
    return;
  }
